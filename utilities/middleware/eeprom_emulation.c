...
/*********************************************************************************************************//**
  * @brief
  * @retval EEPROM_EMU_OPERATION_OK, EEPROM_EMU_NO_ACTIVE_PAGE, EEPROM_EMU_FLASH_ERROR,
            EEPROM_EMU_PAGE_FULL, EEPROM_EMU_OPERATION_FAIL
  ***********************************************************************************************************/
static EEPROM_EMU_State EEPROM_WriteActivePage(u16 DataAddr, u16 Data)
{
  FLASH_State FlashStatus;
  ...
  /* get active page                                                                                        */
  ActivePage = EEPROM_FindActivePage(EEPROM_EMU_WRITE_PAGE);
  ...
  StopAddr = EEPROM_EMU_START_ADDR + ((u32)(1 + ActivePage) * EEPROM_EMU_PAGE_SIZE);   // page end address
  CompareAddr = EEPROM_EMU_START_ADDR + ((u32)ActivePage * EEPROM_EMU_PAGE_SIZE + 4);  // start from page head
